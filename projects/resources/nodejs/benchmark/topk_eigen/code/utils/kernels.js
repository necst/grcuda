const AXPB_XTENDED = (inputType = "float", middleType = "float", outputType = "float") => `
#include "/usr/local/cuda/include/cuda_fp16.h"


extern "C" __global__ void axpb_xtended(const ${inputType} alpha, const ${inputType} *x, const ${inputType} *b, const ${inputType} beta, const ${outputType} *c, ${outputType} *out, const int N, const int offset_x, const int offset_c) {
...
const NORMALIZE = (inputType = "float", middleType = "float", outputType = "float") => `
#include "/usr/local/cuda/include/cuda_fp16.h"

extern "C" __global__ void normalize(const ${inputType} *d_v_in, const double denominator, ${outputType} *d_v_out, const int N) {
    int init = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    for (int i = init; i < N; i += stride) {
        d_v_out[i] = ${outputType}(${middleType}(d_v_in[i]) * ${middleType}(denominator));
    }
}
`

const STORE_AND_RESET = (inputType = "float", middleType = "float", outputType = "float") => `
extern "C" __global__ void store_and_reset(const ${inputType} *to_store, ${outputType} *to_reset, ${outputType} *store_destination, int N, int offset) {
    int init = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    for (int i = init; i < N; i += stride) {
        store_destination[offset + i] = ${outputType}(to_store[i]);
        to_reset[i] = ${outputType}(0.0);
    }
}
`

const COPY_PARTITION_TO_VEC = (inputType = "float", middleType = "float", outputType = "float") => `
#include "/usr/local/cuda/include/cuda_fp16.h"

extern "C" __global__ void copy_partition_to_vec(const ${inputType} *vec_in, ${outputType} *vec_out, const int N, const int offset_out, const int offset_in){
    int init = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    for(int i = init; i < N; ++i){
        vec_out[i + offset_out] = ${outputType}(vec_in[i + offset_in]);
    }
}
`

const SQUARE = (inputType = "float", middleType = "float", outputType = "float") => `
#include "/usr/local/cuda/include/cuda_fp16.h"
...
const SPMV = (inputType = "float", middleType = "float", outputType = "float") => `
#include "/usr/local/cuda/include/cuda_fp16.h"

extern "C" __global__ void spmv(const int *x, const int *y, const ${inputType} *val, const ${inputType} *v_in, ${outputType} *v_out, int num_nnz) {
    int init = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;

    for (int i = init; i < num_nnz; i += stride) {
        v_out[y[i]] += ${outputType}(${middleType}(v_in[x[i]]) * ${middleType}(val[i]));
    }
}
`

const SUBTRACT = (inputType = "float", middleType = "float", outputType = "float") => `
#include "/usr/local/cuda/include/cuda_fp16.h"

extern "C" __global__ void subtract(${inputType}* v1, const ${inputType}* v2, const ${inputType} alpha, int N, int offset) {
...
const DOT_PRODUCT_STAGE_ONE = (inputType = "float", middleType = "float", outputType = "float") => `
#include "/usr/local/cuda/include/cuda_fp16.h"

__global__ void dot_product_stage_one(const ${inputType}* v1, const ${inputType}* v2, ${outputType}* temporaryOutputValues, int N, int offset) {
    extern __shared__ ${outputType} cache[];
    int threadId = threadIdx.x + blockIdx.x * blockDim.x;
    int cacheIdx = threadIdx.x;
    
    int stride = blockDim.x * gridDim.x;
    ${outputType} temp = ${outputType}(0.0);
    for(int i = threadId; i < N; i += stride){
        temp += ${outputType}(${middleType}(v1[i]) * ${middleType}(v2[i + offset]));
    }
...
const DOT_PRODUCT_STAGE_TWO = (inputType = "float", middleType = "float", outputType = "float") => `
#include "/usr/local/cuda/include/cuda_fp16.h"

extern "C" __global__ void dot_product_stage_two(const ${inputType} *temporary_results, ${outputType} *result) {
    
    ${middleType} acc = ${middleType}(temporary_results[threadIdx.x]);
    for(int i = 16; i > 0; i >>= 1){
        acc += ${middleType}(__shfl_down_sync(0xffffffff, acc, i));
    }
    
    __syncthreads();
    
    if(threadIdx.x == 0) *result = ${outputType}(acc);
}
`
...
const _WARP_REDUCE = (outputType = "float") => `
__inline__ __device__ ${outputType} warp_reduce(${outputType} val) {
    int warp_size = 32;
...
const L2_NORM = (inputType = "float", middleType = "float", outputType = "float") => `
#include "/usr/local/cuda/include/cuda_fp16.h"

${_WARP_REDUCE(inputType)}

extern "C" __global__ void l2_norm(const ${inputType} *x, ${outputType} *z, const int N, const int offset) {
    int warp_size = 32;
    float sum = 0;
...
const DOT_PRODUCT = (inputType = "float", middleType = "float", outputType = "float") =>  `
#include "/usr/local/cuda/include/cuda_fp16.h"

${_WARP_REDUCE(inputType)}

extern "C" __global__ void dot_product(const ${inputType} *x, const ${inputType} *y, ${outputType} *z, const int N, const int offset) {
    int warp_size = 32;
    ${outputType} sum = 0;
    for (int i = blockIdx.x * blockDim.x + threadIdx.x; i < N; i += blockDim.x * gridDim.x) {
        sum += ${outputType}(${middleType}(x[i]) * ${middleType}(y[i + offset]));
    }
    sum = warp_reduce(sum); // Obtain the sum of values in the current warp;
    if ((threadIdx.x & (warp_size - 1)) == 0) // Same as (threadIdx.x % warp_size) == 0 but faster
        atomicAdd(z, sum); // The first thread in the warp updates the output;
...
const CAST = (inputType = "float", outputType = "float") => `
#include "/usr/local/cuda/include/cuda_fp16.h"

extern "C" __global__ void device_cast(const ${inputType}* v1, ${outputType}* v2, const int N) {
    int init = threadIdx.x + blockIdx.x * blockDim.x;
    int stride = blockDim.x * gridDim.x;
