...
class Jacobi {
    ...
    build = (tm) => {
        ...
        this.originalMatrix = createMatrix(this.N, this.N)
    ...
    _truncate = (tol) => {
        for(let i = 0; i < this.N; ++i){
            for(let j = 0; j < this.N; ++j){
                if(i == j) continue
                if(this.matrix[i][j] < tol) this.matrix[i][j] = 0.0
    ...
    _computeReconstructionAccuracy = () => {
        let diff = 0.0
        for(let i = 0; i < this.N; ++i){
            const eigenvalue = this.matrix[i][i]
            const eigenvector = this.eigenvectors[i]
            // Perform Av - lambdaV
            const LambdaV = eigenvector.map(v => v * eigenvalue)
            const Av = matmul(this.originalMatrix, eigenvector)

            diff += Av.reduce((acc, cur, idx) => acc + (cur - LambdaV[idx]))

        }
        // return the average
        return diff / this.N / this.N
    }

    printResults = () => {
        const reconstructionAccuracy = this._computeReconstructionAccuracy()
        for(let i = 0; i < this.N; ++i) {
            arrayNormalize(this.eigenvectors[i])
        }
        ...
        for (let i = 0; i < this.N; ++i) {
            for (let j = 0; j < this.N; ++j) {
                if (i == j) continue
                orthogonality += dotProduct(this.eigenvectors[i], this.eigenvectors[j])
            }
        }
        orthogonality = orthogonality / (this.N * this.N )
        orthogonality = Math.abs(Math.acos(orthogonality) * 180 / Math.PI - 90)
        process.stdout.write(`,${orthogonality},${reconstructionAccuracy},`)
        return this
    }
