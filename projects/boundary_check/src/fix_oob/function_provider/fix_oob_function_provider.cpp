...
bool FixOOBFunctionProvider::add_array_access_protection(
    ...
    for (auto access : array_accesses_postprocessed) {
        ...
        add_array_size_if_statement(access, start_if_block, end_if_block, protect_lower_bounds, positive_cuda_dependent_values);
...
void FixOOBFunctionProvider::add_array_size_if_statement(
    ArrayAccess *access,
    BasicBlock *start_if_block,
    BasicBlock *end_if_block,
    bool protect_lower_bounds,
    std::set<Value *> &positive_cuda_dependent_values,
    CmpInst::Predicate comparison
    ) {
    // Delete the unconditional branch added by the block creation;
    // Instruction *start_if_instruction = access->start; 
    // start_if_instruction->getParent()->getPrevNode()->getTerminator()->eraseFromParent();
    start_if_block->getPrevNode()->getTerminator()->eraseFromParent();
    ...
    IRBuilder<> builder(start_if_block->getPrevNode());
