...
void convolution_cpu(float *x, float *y, int size, float *res) {
    for (int i = 0; i < size; i++) {
        *res += x[i] * y[size - i - 1];
    }
}

extern "C" __global__ void convolution(float *x, float *y, int size, float *res) {
    __shared__ float cache[NUM_THREADS];
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    // This OoB access doesn't affect the result;
    cache[threadIdx.x] = x[i] * y[size - i - 1];
    __syncthreads();
...
extern "C" __global__ void convolution_checked(float *x, float *y, int size, float *res) {
    __shared__ float cache[NUM_THREADS];
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i < size) {
        cache[threadIdx.x] = x[i] * y[size - i - 1];
...
int main(int argc, char *argv[]) {
    ...
    convolution_cpu(x, y, size, &res_gold);
    ...
    convolution_checked<<<num_blocks, NUM_THREADS>>>(x_d, y_d, size, res_d);
    end = clock_type::now();
    ...
    start = clock_type::now();
    convolution<<<num_blocks, NUM_THREADS>>>(x_d, y_d, size, res_d);
    end = clock_type::now();
    duration = chrono::duration_cast<chrono::milliseconds>(end - start).count();
    cudaMemcpy(&res, res_d, sizeof(float), cudaMemcpyDeviceToHost);
    cudaCheckError();
