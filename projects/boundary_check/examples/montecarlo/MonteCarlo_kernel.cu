    ...
    int optionN)
{
    ...
    // Here, the number of blocks is equal to the number of options, so we never OoB access rngStates
    ...
    for(int optionIndex = blockIdx.x; optionIndex < optionN; optionIndex += gridDim.x)
    {
        ...
        for (int iSum = threadIdx.x; iSum < SUM_N; iSum += blockDim.x)
        {
            ...
            // Here, the value of iSum is protected by a for loop
        ...
        // This is similar to the other example;
