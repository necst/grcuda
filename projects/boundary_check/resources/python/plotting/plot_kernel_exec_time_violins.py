...
#     plt.savefig(os.path.join(PLOT_FOLDER, "exec_times_ridge_2c.pdf"))
#     plt.savefig(os.path.join(PLOT_FOLDER, "exec_times_ridge_2c.png"))
...
#    res_summary["kernel"] = "Summary, Geomean"
#    res_tot = pd.concat([res_summary, res_tot])
    g = sns.FacetGrid(res_tot, row="kernel_group", hue="kernel", aspect=10, height=0.8, palette=["#2f2f2f"], sharey=False,
                      col="group")
    g.map(plt.axvline, x=1, lw=0.75, clip_on=True, zorder=0, linestyle="--", ymax=0.5)                                                                                          
    g.map(sns.kdeplot, "time_u_k_ms", clip_on=False, clip=x_lim, shade=True, alpha=0.6, lw=1, bw=0.25, color=b1, zorder=2, cut=10)  
    g.map(sns.kdeplot, "time_m_k_ms", clip_on=False, clip=x_lim, shade=True, alpha=0.6, lw=1, bw=0.25, color=r1, zorder=3, cut=10)
    g.map(sns.kdeplot, "time_u_k_ms", clip_on=False, clip=x_lim, color="w", lw=1.5, bw=0.25, zorder=2, cut=10)
    g.map(sns.kdeplot, "time_m_k_ms", clip_on=False, clip=x_lim, color="w", lw=1.5, bw=0.25, zorder=3, cut=10)
...
#    g.fig.get_axes()[-1].axvline(x=1, ymax=0.5)
    ...
    g.fig.suptitle("Kernel Relative\nExec. Time Distribution,\nO2 Opt. Level", ha="left", x=0.05, y=0.95, fontsize=18)
    
    plt.savefig(os.path.join(PLOT_FOLDER, "exec_times_ridge.pdf"))
    plt.savefig(os.path.join(PLOT_FOLDER, "exec_times_ridge.png"))
